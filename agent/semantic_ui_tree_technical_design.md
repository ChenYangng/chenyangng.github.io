...
第一类是 **当前页面状态信号**，它基于当前静态页面或当前窗口状态即可得到，例如：

- 当前 Activity/window/page 标识
- 当前输入焦点
- 当前滚动位置
- 键盘是否弹出
- 弹窗、toast、loading 是否存在
- 页面是否稳定，是否仍在动画或加载中
- 可点击节点、可输入节点、禁用节点的当前状态

第二类是 **交互历史与 action transition 信号**，它必须依赖历史事件，通常需要执行 action 后才能得到，例如：

- 用户或 Agent 的点击、长按、滚动、输入、返回等事件
- 动作发生前后的控件树 diff、截图 diff、窗口/页面标识变化
- 输入焦点变化、弹窗出现或消失、toast/loading 状态变化、列表滚动位置变化
- 如果系统能感知执行结果，还可以包括动作成功、失败、超时、被拦截等回执

当前静态页面只能提供第一类状态信号，无法直接提供动作后果。第二类 transition 信号主要用于离线语义编译、Action Graph 构建和执行反馈回流，例如推断 `tap card_body -> open_detail`、`tap close_icon -> dismiss_overlay`。
...
“动作执行后的页面变化和动作回执信号”指的是动作执行后的结果观测，例如点击某个节点后是否进入新页面、是否弹出弹窗、是否关闭浮层、是否只是选中某个 tab、是否出现错误提示。这类信号基于当前静态页面无法直接得到，需要通过历史轨迹或真实执行后的反馈获得，是构建 Action Graph 和执行反馈回流的关键依据。
...
语义化控件树不适合只做在单个 App SDK 中，也不适合仅做上层工具逻辑。

更合适的方式是：

> 将其设计为系统级 `Semantic UI Service`，位于无障碍/自动化基础能力之上，对 Agent、测试、RPA、无障碍工具统一提供接口。

建议输出能力包括：

- `getObservationPack()`
- `getNormalizedTree()`
- `getSemanticObjectGraph()`
- `getUIState()`
- `getUIChangeSummary()`
...
热启动路径的目标是在 1s 内生成可用的语义化控件树，并将其交给 GUI Agent 或小模型进行下一步决策。
...
| 语义模型与规则资产 | 云侧训练，端侧下发 | 对象识别、节点角色、动作语义、风险判断 | 低 |
